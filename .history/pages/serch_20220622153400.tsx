...
function Serch() {
  ...
  const numV = (event: MouseEvent<HTMLButtonElement>) => {
...
　 //mapのバリューはnumber型しか設定できず
  const pageN = newLinks.map((num:number , i:number) => {
    return (
       //buttonタグのidにはstring型しか入らない
      <button id={String(num)} key={i} onClick={numV} >
