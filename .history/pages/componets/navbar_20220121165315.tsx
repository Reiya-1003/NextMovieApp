    ...
    <div className={`${styles.nav} ${show && styles.nav__black}`}>
         ...
         <div className={styles.nav__contents}>
           ...
           <form action="" className={styles.nav__serchbox}>
              ...
              <button  value="検索"　onClick={handleSubmit} style={{marginLeft:"1%"}}>検索</button>
      ...
      <button onClick={()=>signIn()} style={{marginRight:"-50px"}}>Sign In</button>
      </div>}
      {session && <div style={{display:"flex",backgroundColor:"transparent",fontSize:"small"}}><div className={styles.nav__list}>{session.user.name}</div>
      <div style={{backgroundColor:"transparent"}}><button onClick={()=>signOut()} style={{color:"gray",backgroundColor:"transparent"}}> Sign Out</button></div></div>}
      </span>
