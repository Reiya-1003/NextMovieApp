           ...
           <div className={styles.title__cavas__Con}>
            　　 {/* ifぶん */}
               ...
               <div className={styles.title__cavas__detail}>
                 <div className={styles.title__cavas__name}>
                       {movieDetail.original_title}
                 </div>
                 <div className={styles.title__cavas__buttonarea}>
                   {session && togllebuton.length === 0 && (
                      <button  onClick={addFavolist} className={styles.title__cavas__button}>
                          <AddCircleOutlineIcon/>
                           AddFavo
                      </button>
                        )}
                  {session && togllebuton.length === 1 && (
                       <button  onClick={()=>handleDelete(togllebuton[0].id)} className={styles.title__cavas__button}>
                            <RemoveCircleOutlineIcon/>
                            DeleteFavo
                         </button>
                        )}
                    <div className={styles.title__cavas__button}><a>{movieDetail.genres[1]}</a></div>
