...
type Session = { data: Session; status: "authenticated"; } | 
...
export default function Favo() {
  ...
  const { data: session, status }:undefined = useSession();
...
//
  let userlist = currentMovie.filter(function (e:Userlist) {
    if (status === "authenticated") return e.userId === session.user.id
  });
