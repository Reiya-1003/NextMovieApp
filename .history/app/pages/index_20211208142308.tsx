...
export default function Home({ posts }: any) {
  ...
  const {movieid, setMovie}= useContext(MyIdContext)
  console.log(movieid)
          ...
          return (
            <li key={post.id}>
              <Link href={`/details/${post.id}`}>
                <a onClick={()=>{setMovie(post.id)}}>{post.title}</a>
              </Link>
              <img　src={IMG_PATH + post.poster_path}
                    style={{ width: "100px", height: "200px" }}></img>
            </li>
          );
        })}}
